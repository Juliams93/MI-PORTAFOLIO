...
const products = [
  { number: "01", name: "FamilyHOME", eyebrow: "Producto funcional · Smart home", summary: "Un centro privado para acompañar y cuidar a la familia mediante cámaras domésticas, transmisión local y una experiencia sencilla.", challenge: "Reunir dispositivos reales en una interfaz accesible sin publicar credenciales ni imágenes familiares.", features: ["Streaming local protegido", "Cámaras Tapo y PTZ", "Acceso privado", "Diseño multidispositivo"], stack: ["React", "Vite", "WebRTC", "MediaMTX", "Clerk"], images: ["/projects/familyhome/dashboard.webp", "/projects/familyhome/settings.webp"], url: "https://familyhome-ochre.vercel.app/", tone: "teal", icon: Home, status: "En uso" },
    ...
    <main>
      ...
      <section className="products section" id="productos">
        ...
        <div className="product-list">{products.map((product, index) => { const Icon = product.icon; return <motion.article className={`product-card ${product.tone}`} key={product.name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ delay: index * .08 }}>
          ...
          {product.images && <div className="product-preview">{product.images.map((image, imageIndex) => <img key={image} src={image} alt={`${product.name}: ${imageIndex === 0 ? "panel principal con las cámaras del hogar" : "configuración de cámaras y modo noche"}`} loading={imageIndex === 0 ? "eager" : "lazy"}/>)}</div>}
